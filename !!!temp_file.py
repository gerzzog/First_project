#Это черновой файл для получения 3 вьюх, чтобы было понимание, что код работает
#Нужно только сохранить файл CEO.CSV в папку "/lessons/geo.csv" и скрипт заработает
#В папке scripts сохранил итоговый вариант, который работает с Дагом
...
#ссылка на файл с координатами городов
cities = "/lessons/geo.csv"
...
from pyspark.sql.functions import regexp_replace, datediff,countDistinct,from_utc_timestamp, concat, lit, to_date, weekofyear, date_format, current_date,current_timestamp
...
def input_event_paths(date, depth):
    ...
    return [f"/user/master/data/geo/events/date={(dt - datetime.timedelta(days=x)).strftime('%Y-%m-%d')}" for x in
            range(depth)]

#берем диапазон на дату с глубиной 1 день
reaction_paths = input_event_paths('2022-04-05', 5)
#читаем файл с данными по координатам городов
pandas_data_frame = pd.read_csv(cities, sep = ';')
